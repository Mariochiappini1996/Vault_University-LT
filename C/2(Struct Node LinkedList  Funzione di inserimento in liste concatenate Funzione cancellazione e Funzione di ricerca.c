...
linked_list insert(linked_list, int, float); //Prototipo Funzione di Inserimento
linked_list delete(linked_list, int);//Prototipo Funzione di Cancellazione
...
linked_list delete(linked_list L, int pos){ //Funzione di cancellazione
	node *q, *p;
	if (pos > 0 && pos < L.n){
		q = search(L, pos-1);
		p = q->next;
...
linked_list insert(linked_list L, int pos, float e){ //Funzione Inserimento
	node *q, *p;
	if (pos > 0 && pos <= L.n) {
		q = search(L, pos-1);
