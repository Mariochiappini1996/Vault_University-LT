...
object new_string_alias(char *v){
	object ob = {'s', NULL};
	ob.pointer = v; /* ob.pointer Ã¨ un alias, complessitÃ  temporale e spaziale  O(1)  */
	/*
	ob.pointer = malloc(sizeof(char));
	*((char*)ob.pointer) = v;*/
	return ob;
...
object new_string_clone(char *v){
	object ob = {'s', NULL}; /* ob.pointer contiene una copia, complessitÃ  temporale e spaziale  O(strlen(v))  */
	ob.pointer = malloc(sizeof(char)*(strlen(v)+1));
